// (c) github.com/jazzycamel/PS2Mouse
#include "PS2Mouse.h"
...
void PS2Mouse::begin(void){
  write(0xFF);  // reset, 100Hz
  for(int i=0; i<2; i++) read();  // JB: changed from 3 reads to 2
  write(0xF0);  // set remote (poll) mode
  read();
  delayMicroseconds(100);
}

void PS2Mouse::getPosition(uint8_t &stat, int &x, int &y){
  write(0xEB);  // read data cmd
  read();
  stat=read();
  uint8_t _x=read();
  uint8_t _y=read();  
